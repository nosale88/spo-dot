...
export function GlobalDataProvider({ children }: { children: ReactNode }) {
  ...
  const loadTasks = async (filters?: FilterOptions) => {
    await withLoading('tasks', async () => {
      const response = await apiService.tasks.getAll(filters);
  ...
  const loadDailyReports = async (filters?: FilterOptions) => {
    await withLoading('dailyReports', async () => {
      const response = await apiService.dailyReports.getAll(filters);
      dispatch({ type: 'SET_DAILY_REPORTS', payload: response.data.data });
    });
  };

  const loadAnnouncements = async (filters?: FilterOptions) => {
    await withLoading('announcements', async () => {
      const response = await apiService.announcements.getAll(filters);
      dispatch({ type: 'SET_ANNOUNCEMENTS', payload: response.data.data });
    });
  };

  const loadManuals = async (filters?: FilterOptions) => {
    await withLoading('manuals', async () => {
      const response = await apiService.manuals.getAll(filters);
      dispatch({ type: 'SET_MANUALS', payload: response.data.data });
    });
  };

  const loadSales = async (filters?: FilterOptions) => {
    await withLoading('sales', async () => {
      const response = await apiService.sales.getAll(filters);
      dispatch({ type: 'SET_SALES', payload: response.data.data });
    });
  };

  const loadCustomers = async (filters?: FilterOptions) => {
    await withLoading('customers', async () => {
      const response = await apiService.customers.getAll(filters);
      dispatch({ type: 'SET_CUSTOMERS', payload: response.data.data });
    });
  };

  const loadSuggestions = async (filters?: FilterOptions) => {
    await withLoading('suggestions', async () => {
      const response = await apiService.suggestions.getAll(filters);
      dispatch({ type: 'SET_SUGGESTIONS', payload: response.data.data });
    });
  };
  ...
  const createTask = async (task: Partial<Task>) => {
    const response = await apiService.tasks.create(task);
    dispatch({ type: 'ADD_TASK', payload: response.data });
  };

  const updateTask = async (id: string, task: Partial<Task>) => {
    const response = await apiService.tasks.update(id, task);
  ...
  const createDailyReport = async (report: Partial<DailyReport>) => {
    const response = await apiService.dailyReports.create(report);
    dispatch({ type: 'ADD_DAILY_REPORT', payload: response.data });
  };

  const updateDailyReport = async (id: string, report: Partial<DailyReport>) => {
    const response = await apiService.dailyReports.update(id, report);
    dispatch({ type: 'UPDATE_DAILY_REPORT', payload: response.data });
  };

  const createAnnouncement = async (announcement: Partial<Announcement>) => {
    const response = await apiService.announcements.create(announcement);
  ...
  const updateAnnouncement = async (id: string, announcement: Partial<Announcement>) => {
    const response = await apiService.announcements.update(id, announcement);
  ...
  const createManual = async (manual: Partial<Manual>) => {
    const response = await apiService.manuals.create(manual);
    dispatch({ type: 'ADD_MANUAL', payload: response.data });
  };

  const updateManual = async (id: string, manual: Partial<Manual>) => {
    const response = await apiService.manuals.update(id, manual);
    dispatch({ type: 'UPDATE_MANUAL', payload: response.data });
  };

  const deleteManual = async (id: string) => {
    await apiService.manuals.delete(id);
