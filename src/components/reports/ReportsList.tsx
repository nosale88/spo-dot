          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              ...
              className={clsx(
                ...
                sortBy === 'updatedAt' ? 'bg-slate-200 dark:bg-slate-600' : 'bg-slate-100 dark:bg-slate-700'
              )}
            >
              최근 수정순
              {sortBy === 'updatedAt' && (
            ...
            <button
              ...
              className={clsx(
                'px-3 py-1 rounded-lg text-sm flex items-center',
                sortBy === 'createdAt' ? 'bg-slate-200 dark:bg-slate-600' : 'bg-slate-100 dark:bg-slate-700'
              )}
            >
              작성일순
              {sortBy === 'createdAt' && (
            ...
            <button
              ...
              className={clsx(
                'px-3 py-1 rounded-lg text-sm flex items-center',
                sortBy === 'title' ? 'bg-slate-200 dark:bg-slate-600' : 'bg-slate-100 dark:bg-slate-700'
              )}
            >
              제목순
              {sortBy === 'title' && (
        ...
        {filteredReports.length > 0 ? (
          filteredReports.map(report => (
            <div
              key={report.id}
              className={clsx(
                "bg-white dark:bg-slate-800 rounded-lg p-4 shadow-sm cursor-pointer transition-all hover:shadow",
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-start space-x-3">
                  <div className={clsx(
                    ...
                    report.status === 'reviewed' && "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300",
                      ...
                      {report.tags && report.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {report.tags.slice(0, 2).map(tag => (
                            <span key={tag} className="bg-slate-100 text-slate-700 text-xs px-2 py-0.5 rounded-full dark:bg-slate-700 dark:text-slate-300">
                              {tag}
                            </span>
                          ))}
